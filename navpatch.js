...
/* Maps sub-nav strip: add newer maps to the hardcoded .mapsub bar on the older map
   pages, so a new map only has to be added here and in nav-config.js. */
(function(){try{
  var strip=document.querySelector('.mapsub'); if(!strip) return;
  var cur=(location.pathname.split('/').pop()||'index.html');
  var EXTRA=[{href:'data-flow-map.html',label:'Data Flow Map'}];
  var ver=strip.querySelector('.mapsub-ver');
  EXTRA.forEach(function(it){
    if(strip.querySelector('a[href="'+it.href+'"]')) return;
    var a=document.createElement('a');
    a.setAttribute('href',it.href); a.textContent=it.label;
    if(it.href===cur) a.className='on';
    if(ver) strip.insertBefore(a,ver); else strip.appendChild(a);
  });
